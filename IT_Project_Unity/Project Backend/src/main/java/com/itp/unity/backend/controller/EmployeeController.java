...
public class EmployeeController {
    ...
    @GetMapping("/employees/designation/{id}")
 	public @ResponseBody ResponseEntity<?> getEmployeeDesinationById(@PathVariable Long id) {

 		try {
 			if (employeeService.getEmployeeById(id) != null) {
 				return new ResponseEntity<>(employeeService.getEmployeeDesinationById(id), HttpStatus.OK);
 			} else {
 				return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);
 			}
    ...
    public  @ResponseBody ResponseEntity<?> findByDesignation(@RequestParam(required = false) String designation) {
        try {
            List<Employee> employee = new ArrayList<>();
            employee = employeeService.findByDesignation("Doctor");
            if(employee != null) {
